...
3. Open the SQL Editor and run [`schema.sql`](./schema.sql), then run [`youtube_upgrade.sql`](./youtube_upgrade.sql).
...
The underlying tables cannot be accessed directly by anonymous visitors. Posts and reports are rate-limited in the database. Spotify and YouTube embed URLs are reconstructed server-side from validated item IDs, and each post can attach at most one media provider.
