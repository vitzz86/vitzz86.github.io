...
function renderProfile(record) {
  ...
  const companyBody=`<nav class="map-profile-icon-row" aria-label="Company links">${profileLinks(record)}</nav><p class="map-profile-offering">${escapeHTML(profile.description || record.offering)}</p><dl class="map-profile-facts">${profileFacts(record)}</dl>${leaderMarkup(record)}`;
